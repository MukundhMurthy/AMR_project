...
def train_parser():
    ...
    parser.add_argument("--batch_size", type=int, default=32, help="batch size for training set")
    parser.add_argument("--hidden", type=int, default=512, help="# of hidden layers for transformer block")
    ...
    parser.add_argument("--depth", type=int, default=3, help='# of transformer block repeats')
    ...
    parser.add_argument("--patience", type=int, default=2, help="num epochs for lr scheduler to wait before decreasing")
    ...
    parser.add_argument('--num_workers', type=int, help="number of cpu workers for dataloaders", default=1)
    parser.add_argument('--job-dir', help='GCS location to export models')
    parser.add_argument('--es_patience', type=int, default=4, help='Early stopping patience')
...
def cscs_parser(train_parser):
    ...
    parser.add_argument('--POI_file', default="escape_validation/regions_of_interest.json", help='positions of interest file')
    parser.add_argument('--wt_seqs_file', default="escape_validation/anchor_seqs.fasta", help='wildtype sequences for baseline embeddings and probability prior')
    ...
    parser.add_argument('--scaling', default='min_max', help='scaling to apply to cscs, semantics, and grammar',
...
def cscs_calc(arg, data, eval_model, state_dict_fname, model_type='attention'):
    ...
    metrics = Metrics(state_dict_fname, arg.wt_seqs_file, arg.file_column_dictionary, results_fname=arg.name_run, job_dir=arg.job_dir, wandb=arg.wandb,
...
if __name__ == '__main__':
    ...
    if not args.benchmark and not (args.calc_metrics and not (args.train or args.cscs)):
